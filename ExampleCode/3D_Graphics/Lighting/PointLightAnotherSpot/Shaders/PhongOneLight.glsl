...
/**
Fragment shader that uses the Phong lighting model with a single
light source.
...
void main()
{
    ...
    if (Lt.on)  // Color has been replaced by Material Properties
    {
        ...
        if (spotCos < SpotCosCutoff && Lt.spotCutoff < 179.9)  // Only fade if a spotlight
        {
            float range = 1 + SpotCosCutoff;
            spotFactor = pow(1 - (SpotCosCutoff - spotCos)/range, Lt.spotExponent);
        }
