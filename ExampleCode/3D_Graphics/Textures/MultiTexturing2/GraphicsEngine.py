...
class GraphicsEngine():
    ...
    def __init__(self):
        ...
        self.plane = SimplePlane()

        # glActiveTexture(GL_TEXTURE0)

        glUseProgram(self.TextureShader)

        # Turn off all 10 texture slots.
        for i in range(10):
            glUniform1i(glGetUniformLocation(self.TextureShader, "useTexture[" + str(i) + "]"), False)

        # Create a list of texture addresses for convenience.
        textures = []
        textures.append(self.loadTexture("Images/cat003.png"))
        textures.append(self.loadTexture("Images/metal024.bmp"))
        textures.append(self.loadTexture("Images/Repeat-brick.jpg"))

        # Activate texture units 0, 1, and 2.  Bind a texture to them. Set the shader tex array to see
        # units 0, 1, and 2.  Enable the three texture slots in the shader useTexture array.
        for i in range(3):
            glUniform1i(glGetUniformLocation(self.TextureShader, "useTexture[" + str(i) + "]"), True)
            glUniform1i(glGetUniformLocation(self.TextureShader, "tex[" + str(i) + "]"), i)  # GL_TEXTUREi
