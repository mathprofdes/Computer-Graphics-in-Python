...
class GraphicsEngine():
    ...
    def __init__(self):
        ...
        self.constantColorLoc = glGetUniformLocation(self.StructProgram, "info.constantColor")
        self.passLoc = glGetUniformLocation(self.StructProgram, "info.pass")
        self.projLoc = glGetUniformLocation(self.StructProgram, "Mats.Projection")
        self.rotateLoc = glGetUniformLocation(self.StructProgram, "Mats.Rotation")
        self.scaleLoc = glGetUniformLocation(self.StructProgram, "Mats.Scale")
        self.translateLoc = glGetUniformLocation(self.StructProgram, "Mats.Translate")
        ...
        glUseProgram(self.StructProgram)
        glUniformMatrix4fv(self.rotateLoc, 1, GL_FALSE, glm.value_ptr(RotationMatrix))
        glUniformMatrix4fv(self.scaleLoc, 1, GL_FALSE, glm.value_ptr(ScaleMatrix))
        glUniformMatrix4fv(self.translateLoc, 1, GL_FALSE, glm.value_ptr(self.TranslateMatrix))

        glUniform4fv(self.constantColorLoc, 1, [0, 1, 0, 1])
        glUniform1i(self.passLoc, 1)
    ...
    def update(self):
        ...
        glUniform1i(self.passLoc, 0)
        glUniformMatrix4fv(self.translateLoc, 1, GL_FALSE, glm.value_ptr(self.TranslateMatrix))
        self.circle.draw()

        glUniform1i(self.passLoc, 1)
        glUniformMatrix4fv(self.translateLoc, 1, GL_FALSE, glm.value_ptr(self.TranslateMatrix2))
        self.box.draw()
    ...
    def setProjectionMatrices(self, size):
        ...
        self.setProjectionMatrix(size, self.StructProgram, "Mats.Projection")
